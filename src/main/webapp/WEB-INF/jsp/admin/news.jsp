			...
			$('#edit_submit_news').click(function(){
				$('#editForm').form({
					onSubmit:function(){
						...
						var b=true;
						if($.trim($('#content').val()).length>$('#content').attr('maxLen')){
			...
			<form:form modelAttribute="news" id="editForm" action="${ctx}/admin/news/save" method="post" cssStyle="padding:10px 20px;">
				<table>
					<tr>
						...
						<td><form:input path="title" id="title_edit" required="true"/></td>
						<td><form:label	for="type" path="type" cssClass="mustInput">类别：</form:label></td>
						<td><form:input path="type" id="type_edit" required="true" cssStyle="width:185px;"/></td>
					</tr>
					<tr>
						<td><form:label	for="language" path="language" cssClass="mustInput">语言：</form:label></td>
						<td><form:input path="language" id="language_edit" required="true" cssStyle="width:185px;"/></td>
						<td><form:label	for="status" path="status" cssClass="mustInput">状态：</form:label></td>
						<td><form:input path="status" id="status_edit" required="true" cssStyle="width:185px;"/></td>
					</tr>
